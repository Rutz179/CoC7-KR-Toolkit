...
Hooks.on("createChatMessage", message => {
  ...
  panels.whisper.refresh();
});

Hooks.on("deleteChatMessage", () => {
  if (panels.whisper.open) panels.whisper.refresh();
  if (panels.notes.open) panels.notes.refresh();
});

// Notes are chat messages, so the panel follows the same document events.
Hooks.on("createChatMessage", message => {
  if (panels.notes.open && message.getFlag(MODULE_ID, NOTE_FLAG)) panels.notes.refresh();
});
