...
async function _syncStageMediaTiles(scene = canvas?.scene, stageOverride = null) {
  ...
  // Keep the native Scene background as a simple neutral backing color only.
  if (scene.backgroundColor !== (media.backgroundColor || "#111111")) {
...
class Coc7KoStageOverlay {
  ...
  fitPortraits() {
    ...
    if (!enabled) {
      if (current !== 1) el.style.setProperty("--coc7ko-fit-scale", "1");
      return;
    }
    ...
    for (const [upper, middle, lower] of columns) {
      ...
      for (const node of Object.values(occupied)) {
        if (node) naturalHeight = Math.max(naturalHeight, node.offsetHeight / current);
      }

      // Largest card height h that keeps each occupied neighbour pair apart.
      if (occupied.upper && occupied.middle) {
        limit = Math.min(limit, (height / 2 - top - gap) / 1.5);
      }
      if (occupied.middle && occupied.lower) {
        limit = Math.min(limit, (height / 2 - bottom - gap) / 1.5);
      }
      if (occupied.upper && occupied.lower && !occupied.middle) {
        limit = Math.min(limit, (height - top - bottom - gap) / 2);
      }
    }

    let fit = 1;
    if (naturalHeight > 0 && Number.isFinite(limit)) {
...
function setSpeaking(userId, speaking) {
  ...
  if (speaking) {
    ...
    speakingTimers.set(slotKey, setTimeout(() => setSpeaking(userId, false), SPEAKING_STALE_MS));
  } else {
